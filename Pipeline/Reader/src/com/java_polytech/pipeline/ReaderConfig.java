...
public class ReaderConfig {
    ...
    public RC process(String filename) {
        ...
        while (this.correctConfig && scanner.hasNext()) {
            ...
            for (ConfigFields prefix : ConfigFields.values()) {
                if (pair[0].equalsIgnoreCase(prefix.asString())) {
                    if (defaultHashMap.containsKey(prefix.asString())) {
                        ...
                        this.correctConfig = false;
                        return new RC(RC.RCWho.READER, RC.RCType.CODE_CONFIG_GRAMMAR_ERROR, "In config file, prefix " + prefix.asString() + " met two times.");
                    }
            ...
            if (!is_line_correct) {
                this.correctConfig = false;
                return new RC(RC.RCWho.READER, RC.RCType.CODE_CONFIG_GRAMMAR_ERROR, "In config file, unknown prefix " + pair[0]);
            }
        ...
        if (defaultHashMap.size() != ConfigFields.values().length) {
            this.correctConfig = false;
            return new RC(RC.RCWho.READER, RC.RCType.CODE_CONFIG_GRAMMAR_ERROR, "In config file, Not enough fields");
        }
